...
,DATE_RANGE_APPT_IDS AS (
	SELECT DISTINCT ala.APPT_ID
	FROM ANA_LOG_APPT ala 
	WHERE ala.CREATE_DTM >= to_date(:i_start_date,'YYYY-MM')
    	AND ala.CREATE_DTM < LAST_DAY(to_date(:i_end_date,'YYYY-MM')) +1
    	AND ala.APPT_TYPE_CD = 'R' 
)
, SHSC_AA_APPTS AS  ( 
	SELECT temp.APPT_ID FROM ( 
		SELECT APPT_ID FROM (
			SELECT ROW_NUMBER() OVER (PARTITION  BY aad.APPT_ID ORDER BY aad.UPDATE_DTM DESC ) rn, aad.APPT_ID,aad.ENCNTR_TYPE_ID 
			FROM ANA_APPT_DETL aad
			WHERE aad.APPT_ID IN (SELECT APPT_ID FROM DATE_RANGE_APPT_IDS apptIds)
			) 
		WHERE rn = 1 AND ENCNTR_TYPE_ID IN (SELECT * FROM SHSC_AA_ENCNTRTYPE_IDS)
	) temp
	INNER JOIN ANA_APPT aa ON aa.APPT_ID = temp.APPT_ID
			AND aa.SITE_ID = DECODE(:i_siteId,'-',aa.SITE_ID,:i_siteId)
			AND aa.APPT_TYPE_CD <> 'N'
			AND aa.CREATE_DTM < LAST_DAY(to_date(:i_end_date,'YYYY-MM')) +1
)
, APPT_COUNTS AS  (
	SELECT TO_CHAR(CREATE_DTM,'MM/YY') AS MONTH_YEAR, COUNT(APPT_ID) AS counts
	FROM ANA_LOG_APPT 
	WHERE APPT_ID IN (SELECT APPT_ID FROM SHSC_AA_APPTS) 
		AND CREATE_DTM >= to_date(:i_start_date,'YYYY-MM')
		AND CREATE_DTM < LAST_DAY(to_date(:i_end_date,'YYYY-MM')) +1
		AND APPT_TYPE_CD = 'R'
