...
WITH ENCNTR_TYPES AS (
SELECT ENCNTR_TYPE_ID,ENCNTR_TYPE_CD FROM CLN_ENCNTR_TYPE cet WHERE cet.SVC_CD ='TBC'
AND cet.ENCNTR_TYPE_CD IN ('TBC_CONS','TBC_TREAT','TBC_IM_CONTACT','TBC_IM_REFERRAL','TBC_IM_SPECIAL','TBC_IM_CHEST','TBC_SKIN','TBC_SPEC','TBC_VAC')
),
SITES AS (
SELECT SITE_ID,SITE_ENG_NAME FROM CMN_SITE cs WHERE cs.SVC_CD ='TBC' AND cs.STATUS ='A' AND nvl(cs.IS_MOCK_UP, 0) = 0 
...
PERIOD_ENCOUNTER AS (
SELECT cs.SITE_ID, ce.CASE_NO,ets.ENCNTR_TYPE_CD FROM CLC_ENCNTR ce
INNER JOIN SITES cs ON cs.SITE_ID = ce.SITE_ID
INNER JOIN PMI_PATIENT pp ON pp.PATIENT_KEY = ce.PATIENT_KEY AND nvl(pp.IS_MOCK_UP,0) = 0
INNER JOIN ENCNTR_TYPES ets ON ets.ENCNTR_TYPE_ID = ce.ENCNTR_TYPE_ID
WHERE ce.ENCNTR_STS <> 'D' AND nvl(ce.IS_CANCEL,0) <> 1 AND ce.SVC_CD='TBC' 
...
TBC_CONS_ENCOUNTER AS (
SELECT cs.SITE_ID,ce.CASE_NO,ce.SDT, ets.ENCNTR_TYPE_CD FROM CLC_ENCNTR ce
INNER JOIN SITES cs ON cs.SITE_ID = ce.SITE_ID
INNER JOIN PMI_PATIENT pp ON pp.PATIENT_KEY = ce.PATIENT_KEY AND nvl(pp.IS_MOCK_UP,0) = 0
INNER JOIN ENCNTR_TYPES ets ON ets.ENCNTR_TYPE_ID = ce.ENCNTR_TYPE_ID
WHERE ce.ENCNTR_STS <> 'D' AND nvl(ce.IS_CANCEL,0) <> 1 AND ce.SVC_CD='TBC' 
AND ce.CASE_NO IS NOT NULL AND ets.ENCNTR_TYPE_CD ='TBC_CONS'
),
TBC_CONS_PERIOD_CASE_NO AS (
SELECT DISTINCT ce.CASE_NO FROM PERIOD_ENCOUNTER ce WHERE ce.CASE_NO IS NOT NULL AND ce.ENCNTR_TYPE_CD ='TBC_CONS'
),
FIRST_VISIT_TEMP AS (
SELECT SITE_ID, count(1) AS FIRST_VISIT FROM (
SELECT ce.*, ROW_NUMBER() OVER (PARTITION BY ce.CASE_NO ORDER BY ce.SDT ASC) AS NUM
FROM TBC_CONS_ENCOUNTER ce 
WHERE EXISTS (SELECT 1 FROM TBC_CONS_PERIOD_CASE_NO cs WHERE ce.CASE_NO = cs.CASE_NO) 
)temp
WHERE NUM = 1 AND SDT >= to_date(i_startDate,'yyyy-mm-dd') and SDT < to_date(i_endDate,'yyyy-mm-dd')+1
GROUP BY SITE_ID
)
SELECT cs.SITE_ENG_NAME AS CENTRE,
nvl(FIRST_VISIT,0) AS FIRST_VISIT ,
